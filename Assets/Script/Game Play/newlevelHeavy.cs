...
public class newlevelHeavy : MonoBehaviour {
	...
	IEnumerator startFollow()
	{
		...
		transform.position=new Vector3(8.5f,transform.position.y, player.transform.position.z-5f);
	}
	void Update () {
			...
			if(playerAhead)
			{
				if(!tiltControl.twoBiker)
				{
					...
					if(transform.position.z-playerTransform.position.z<=-2f ||  transform.position.z-playerTransform.position.z>=2f )
					{
						...
						if (Physics.Raycast (transform.position + new Vector3 (0.6f, 1f, 0.0f), Vector3.right, out hit,4f)) {
							...
							if (hit.collider.tag.Contains("Player") || hit.collider.tag.Contains("car")|| hit.collider.transform.root.name.Contains("bikeParent")) {
				...
				else if(tiltControl.twoBiker)
				{
					...
					if(transform.position.z-playerTransform.position.z<=-2f ||  transform.position.z-playerTransform.position.z>=2f )
					{
						...
						if(playerTransform.position.x>=7.5f)
						{
							xPosition=8.5f;
						}
						else if(playerTransform.position.x>7.25f)
						{
							xPosition=8.5f;
						}
						else
		...
		if (targetObj != null) {
			...
			if (dist > 0 && dist <= 35) {
				//				print (targetObj+ "  "+(transform.position.x - targetObj.transform.position.x) );
		...
		if (isCrashed) {
			isCrashed=false;	
			transform.root.position = new Vector3 (8.5f,0.2f,transform.root.position.z);
			xPosition =8.5f;
	...
	void resetPosition()
	{
		...
		transform.root.position = new Vector3 (8.5f,0.2f,player.transform.root.position.z-100);
		...
		transform.Find ("opponentTallguys").Find ("biker-v3").transform.GetComponent<Renderer>().material.mainTexture = biker [bikerCostume];
		transform.Find ("opponentTallguys").Find ("bike_crb").transform.GetComponent<Renderer>().material.mainTexture = bike [bikeCostume];
		if(bikerCostume<4)
			bikerCostume += 1;
		else
			bikerCostume =0;
	...
	{ 
		if (col.transform.tag.Equals ("car")) {
			transform.Find ("opponentTallguys").GetComponent<Animation>().Play("death",PlayMode.StopAll);
			CancelInvoke("setPos");
			Invoke("setPos",3f);
			itselfCrashed=true;
	...
	void setPos()
	{
		...
		transform.Find ("opponentTallguys").Find ("biker_fat-01").transform.GetComponent<Renderer>().material.mainTexture = biker [bikerCostume];
		transform.Find ("opponentTallguys").Find ("bike_body").transform.GetComponent<Renderer>().material.mainTexture = bike [bikeCostume];
		if(bikerCostume<4)
			bikerCostume += 1;
		else
			bikerCostume =0;
